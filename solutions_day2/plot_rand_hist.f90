...
! 100000 integer random numbers in the range [-4, 7] from the mean.
!
...
! set up histogram
  ...
  do n = 1, num_rand
     call random_int(irand, imin, imax)
     status = fgsl_histogram_increment(h, real(irand(1),fgsl_double))
  end do
  do i = imin, imax
...
! initialize plotting
  ...
  do i = imin, imax
     ...
     call plptex( real(i - imin,plflt)+0.5_plflt, y0(i)+sign(60.0,real(y0(i))), &
