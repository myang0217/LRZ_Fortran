module mod_heat_par
!
!> a version of the stencil evaluation that uses the 
...
!> if this is done, the iteration count is reset
  subroutine heat_ival(fival)
    ...
    do concurrent (j = 2: size(phi,2) - 1)
      block
         real(dk) :: x, y
         integer :: i
         y = dy * real(j - 1,dk)
...
!> number of iterations.
  real(dk) function heat_iter(dt, num)
    ...
    do it=1, num 
       num_iter = num_iter + 1
!!       do concurrent (j=2:size(phi,2) - 1,i=2:size(phi,1) - 1)
!!          phinew(i,j) = phinew(i, j) + dt * ( &
...
!!!           phinew(2:size(phi,1) - 1,j) = phinew(2:size(phi,1) - 1,j) + dt * ( &
!!!                   (phi(3:size(phi,1),j) + phi(1:size(phi,1)-2,j) - & 
!!!                   2.0_dk*phi(2:size(phi,1) - 1,j)) * d2x + &
!!!                   (phi(2:size(phi,1) - 1,j+1) + phi(2:size(phi,1) - 1,j-1) - &
!!!                    2.0_dk*phi(2:size(phi,1) - 1,j)) * d2y )
       end do
       if (it == num) then
          dphimax = maxval(abs(phi(2:size(phi,1)-1,2:size(phi,2)-1) - &
